		...
		| BinOp (binOp, left, right) ->
				...
				let rightType = typeExprHelper right env in
					(match binOp with
						...
						| OpCons -> if rightType = T_List leftType then T_List leftType else raise (Fail "mismatch")
					)
		| UnOp (unOp, uexpr) ->
...
assert(typeExpr (Empty T_Int) = T_List T_Int);;
assert(typeExpr (Empty T_Bool) = T_List T_Bool);;
assert(typeExpr (Empty (T_Fun (T_Int, T_Int))) = T_List (T_Fun (T_Int, T_Int)));;
